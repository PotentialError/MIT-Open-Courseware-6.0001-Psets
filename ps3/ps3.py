...
def get_word_score(word, n):
    ...
    component1 = 0
    for x in word:
        component1+=SCRABBLE_LETTER_VALUES[x]
...
def update_hand(hand, word):
    ...
    for letter in word:
        if new_hand.get(letter, -1) != -1:
            new_hand[letter] = new_hand[letter] - 1
    for key, value in list(new_hand.items()):
        if value == 0:
            del new_hand[key]
    return new_hand
...
def is_valid_word(word, hand, word_list):
    ...
    word = word.lower()
    if '*' in word:
        temp1 = word[0:word.index('*')]
        temp2 = word[word.index('*') + 1:len(word)]
        check = False
        for letter in VOWELS:
            if (temp1 + letter + temp2) in word_list:
                check = True
                break
        if not check:
            return False
    ...
    new_hand = copy.deepcopy(hand)
    for letter in word:
        if new_hand.get(letter, -1) == -1 or new_hand.get(letter, - 1) == 0:
            return False
        else:
            new_hand[letter]-=1
    return True
...
def substitute_hand(hand, letter):
    ...
    if letter in hand:
        ...
        for alph in string.ascii_lowercase:
            if alph not in hand:
                letters_to_pick+=alph
        random_letter = random.choice(letters_to_pick)
        letter_freq = hand[letter]
        del new_hand[letter]
        new_hand.update( {random_letter: letter_freq} )
    return new_hand
...
def play_game(word_list):
    ...
    * Returns the total score for the series of hands
    ...
    word_list: list of lowercase strings
    """
    hands = eval(input("Enter total number of hands: "))
    count = 0
    score = 0
    while count < hands:
        ...
        print("----------", end = '')
        while input("Would you like to replay the hand? ").lower() == "yes":
            hand_score = play_hand(hand, word_list)
            print("----------", end = '')
        print()
        score+=hand_score
