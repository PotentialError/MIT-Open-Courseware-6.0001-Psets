...
class SubMessage(object):
    ...
    def build_transpose_dict(self, vowels_permutation):
        ...
        for i in string.ascii_lowercase:
            if i in "aeiou":
                dict[i] = vowels_permutation[vowelCount]
                vowelCount+=1
            else:
                dict[i] = i
        vowelCount = 0
        for i in string.ascii_uppercase:
            if i in "AEIOU":
                dict[i] = vowels_permutation[vowelCount].upper()
                vowelCount+=1
            else:
                dict[i] = i
        return dict
    ...
    def apply_transpose(self, transpose_dict):
        ...
        for let in self.message_text:
            if let in transpose_dict:
                sub_word = sub_word + transpose_dict[let]
            else:
                sub_word = sub_word + let
        return sub_word
...
if __name__ == '__main__':
    ...
    message = SubMessage("Welcome to my work!")
    permutation = "ouaie"
    enc_dict = message.build_transpose_dict(permutation)
    print("Original message:", message.get_message_text(), "Permutation:", permutation)
    print("Expected encryption:", "Wulcimu ti my wirk!")
    print("Actual encryption:", message.apply_transpose(enc_dict))
    enc_message = EncryptedSubMessage(message.apply_transpose(enc_dict))
    print("Decrypted message:", enc_message.decrypt_message())
    
    message = SubMessage("Comet Pride! Support Computer Science!")
    permutation = "eioua"
    enc_dict = message.build_transpose_dict(permutation)
    print("Original message:", message.get_message_text(), "Permutation:", permutation)
